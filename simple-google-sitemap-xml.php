...
function generate_xmlsitemap() {
     ...
     $file_handler = fopen(WP_PLUGIN_DIR.$filename, "w+");
...
function get_content() {
    ...
    $query = "SELECT year(post_modified) AS y, month(post_modified) AS m, day(post_modified) AS d, ID,post_title, post_modified,post_name, post_type, post_parent FROM $table_name WHERE post_status = 'publish' ORDER BY post_date DESC";
...
$path = get_option( 'siteurl' ).WP_PLUGIN_DIR.'/sitemap.xml';
  ...
  <h2>Google Sitemap XML</h2>
     ...
     <tr>
	  ...
	  <td width="100">
	       ...
	       <option value="montly" <?if(get_option('gsxml_hf')=="montly") {echo 'selected';}?>>montly</option>
     ...
     <p style="font-weight:bold;">General parameters</p>
	  ...
	  <td width="100">
	       ...
	       <option value="montly" <?if(get_option('gsxml_gf')=='montly') {echo 'selected';}?>>montly</option>
     ...
     <input type="hidden" name="page_options" value="gsxml_hp,gsxml_gp,gsxml_hf,gsxml_gf" />
